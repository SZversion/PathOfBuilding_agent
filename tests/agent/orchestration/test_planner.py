...
assert "category" not in plan("모루 한국어 이름 아이템 찾아줘")["steps"][0]["arguments"]
assert plan("투사체 개수가 몇 개야?")["steps"][0]["tool"] == "get_projectile_count"
assert plan("투사체 개수가 몇 개야?")["entities"]["stat"] == "ProjectileCount"
assert plan("저주 한도는 몇 개야?")["steps"][0]["tool"] == "get_curse_limit"
comparison = plan("5) Pconc of Bouncing Swap에서 Greater Volley를 끄면 Poisonous Concoction of Bouncing 딜이 왜 줄어?")
assert comparison["intent"] == "mechanism_compare"
assert comparison["entities"]["skillSetSelector"] == "5) Pconc of Bouncing Swap"
